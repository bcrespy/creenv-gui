...
class GUI extends HUDelement {
   ...
   */
  static generateSliderDom (sliderObj, configObject) {
    let callback = (val) => {
      configObject[sliderObj.property] = val;
    ...
    let slider = new Slider(sliderObj.min, sliderObj.max, sliderObj.step, sliderObj.value, callback, callbackFinished);
   ...
   */
  static generateColorDom (colorObj) {
   ...
   * @return {HTMLElement}
   */
  static generateBoolDom (boolObj, configObject) {
   ...
   * @return {HTMLElement}
   */
  static generateInputDom (inputObj, configObject) {
