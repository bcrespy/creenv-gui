...
class GUI extends HUDElement {
  ...
  constructor (controls, position = GUI.POSITION.TOP_RIGHT, visible = true, parent = document.body) {
    ...
    if (this.knobsMap.length > 0 || this.buttonsMap.length > 0) {
      let midi = new Midi(this.knobsMap, this.buttonsMap);
      midi.init().then(() => {
        console.log("test");
      })
    }
  ...
  getControllerType (member) {
    let property = this.configObject[member.property];
    ...
    if (member.midi !== undefined && member.midi) {
      ...
      if (typeof property === "number") {
        ...
        return "boolean";
      } else if(property instanceof Color || (typeof property === "string" && ColorString.get.rgb(this.configObject[member.property]))) {
        return "color";
    ...
    controls.forEach(member => {
      ...
      // it's controller
        ...
        if (type === "knob" || type === "button") {
          ...
          switch (type) {
            case "knob": 
              controller = new MidiKnobController(this.configObject, member.property, member.name, member.min, member.max, member.step, member.onChanged);
              this.knobsMap.push({
                id: member.id,
            ...
            case "button":
              controller = new MidiButtonController(this.configObject, member.property, member.name, member.onPress);
          ...
          switch (type) {
            ...
            case "color":
              controller = new ColorController(this.configObject, member.property, this.position, member.name, member.options, member.callback, () => {
