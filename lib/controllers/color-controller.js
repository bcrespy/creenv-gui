...
/**
   ...
   */
  setupCordelia () {
    let color = colorstring.get.rgb(this.object[this.property]);
    let colorStr = `rgba(${color.join(', ')})`;
    ...
    this.pickerElement.el.addEventListener("save",  () => {
      this.object[this.property] = 'rgba('+colorstring.get.rgb(this.pickerElement.color).join(', ')+")";
